...
class VocoderBaseTask(BaseTask):
    def __init__(self):
        ...
        if self.max_valid_sentences == -1:
            hparams["max_valid_sentences"] = self.max_valid_sentences = (
                self.max_sentences
            )
    ...
    def train_dataloader(self):
        ...
        return self.build_dataloader(
            train_dataset, True, self.max_sentences, hparams["endless_ds"]
        )
    ...
    def test_step(self, sample, batch_idx):
        ...
        for idx, (wav_pred, wav_gt, item_name) in enumerate(
            ...
            save_wav(
                wav_gt.view(-1).cpu().float().numpy(),
                f"{gen_dir}/{item_name}_gt.wav",
                hparams["audio_sample_rate"],
            )
            save_wav(
                wav_pred.view(-1).cpu().float().numpy(),
                f"{gen_dir}/{item_name}_pred.wav",
                hparams["audio_sample_rate"],
            )
        return loss_output
